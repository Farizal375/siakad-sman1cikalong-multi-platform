      ...
      if (mounted)
        setState(() {
          ...
          if (_selectedKelasId == null && list.isNotEmpty)
            _selectedKelasId = list.first['id'] as String;
        });
    } catch (_) {}
  }
      ...
      children: [
        Column(
          ...
          children: [
            ...
            const Text(
              'Kelola pemetaan guru ke mata pelajaran dan susun jadwal mingguan per kelas',
...
class _TeacherSubjectMappingState extends State<_TeacherSubjectMapping>
    ...
    setState(() {
      _filtered = q.isEmpty
          ...
          : _list
                .where(
                  (m) =>
                      (m['teacher'] ?? '').toString().toLowerCase().contains(
                        q,
                      ) ||
    ...
    showDialog(
      ...
      builder: (_) => Dialog(
        ...
        child: ConstrainedBox(
          ...
          child: GuruMapelFormModal(initialData: data),
        ),
      ),
    ).then((r) {
      if (r == true) {
        widget.onSuccess(
          data != null
              ? 'Pemetaan berhasil diperbarui'
              : 'Pemetaan berhasil ditambahkan',
        );
        _load();
      }
    });
                  ...
                  child: _loading
                      ...
                      : ListView.separated(
                          ...
                          itemBuilder: (_, i) {
                            final m = pageData[i];
                            final scheduled =
                                (m['scheduled'] as num?)?.toInt() ?? 0;
                            final quota =
                                (m['hoursPerWeek'] as num?)?.toInt() ?? 0;
...
class _WeeklyScheduleGridState extends State<_WeeklyScheduleGrid>
    with AutomaticKeepAliveClientMixin {
  ...
  void initState() {
    super.initState();
    _activeKelasId = widget.selectedKelasId;
    if (_activeKelasId != null) _loadJadwal();
  }

  @override
  void didUpdateWidget(_WeeklyScheduleGrid old) {
    super.didUpdateWidget(old);
    if (widget.selectedKelasId != _activeKelasId) {
      _activeKelasId = widget.selectedKelasId;
      _loadJadwal();
    }
  }
  ...
  void _openAddModal({String? prefillDay}) {
    ...
    showDialog(
      ...
      builder: (_) => Dialog(
        ...
        child: ConstrainedBox(
          ...
          child: ScheduleFormModal(
            ...
            initialData: prefillDay != null
                ? {'day': prefillDay, 'hari': prefillDay}
                : null,
            isEdit: false,
          ),
        ),
      ...
      if (r == true) {
        widget.onSuccess('Jadwal berhasil ditambahkan');
        _loadJadwal();
      }
    });
  }
  ...
  void _delete(Map<String, dynamic> entry) {
    DeleteConfirmationModal.show(
      ...
      onConfirm: () async {
        await ApiService.deleteJadwal(entry['id']);
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${widget.entry['teacher'] ?? '-'} • ${widget.entry['startTime'] ?? ''}–${widget.entry['endTime'] ?? ''}',
                          style: const TextStyle(
                            fontSize: 12,
