...
class _GuruMapelFormModalState extends State<GuruMapelFormModal> {
  ...
  List<Map<String, dynamic>> _guruMapelList = [];
  ...
  Future<void> _load() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _guruList = ((results[0]['data'] as List?) ?? [])
              .cast<Map<String, dynamic>>();
          _mapelList = ((results[1]['data'] as List?) ?? [])
              .cast<Map<String, dynamic>>();
          _kelasList = ((results[2]['data'] as List?) ?? [])
              .cast<Map<String, dynamic>>();
          _guruMapelList = ((results[3]['data'] as List?) ?? [])
              .cast<Map<String, dynamic>>();
  ...
  Map<String, String> get _usedClassesForSelectedMapel {
    if (_mapelId == null) return {};
    final used = <String, String>{};
    for (final gm in _guruMapelList) {
      if (gm['subjectId'] != _mapelId) continue;
      if (widget.initialData != null && gm['id'] == widget.initialData!['id']) {
        continue;
      }
      final teacher = (gm['teacher'] ?? 'Guru lain').toString();
  ...
  Future<void> _save() async {
    if (_guruId == null || _mapelId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Guru dan mata pelajaran wajib diisi'),
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text(
                isEdit ? 'Edit Pemetaan Guru' : 'Tambah Pemetaan Guru',
          ...
          if (_loading)
            ...
            DropdownButtonFormField<String>(
              ...
              onChanged: (v) => setState(() {
                _mapelId = v;
                final used = _usedClassesForSelectedMapel;
                _selectedClasses.removeWhere(used.containsKey);
              }),
              ...
              child: _kelasList.isEmpty
                  ...
                  : Wrap(
                      ...
                      children: _kelasList.map((k) {
                        final className =
                            (k['name'] ?? k['nama'] ?? '') as String;
                        final usedBy = _usedClassesForSelectedMapel[className];
                        final isUnavailable = usedBy != null;
                        final isSelected = _selectedClasses.contains(className);
                        return FilterChip(
                          label: Text(
                            isUnavailable ? '$className • $usedBy' : className,
                          ),
                          selected: isSelected,
            ...
            if (_mapelId != null &&
                _usedClassesForSelectedMapel.isNotEmpty) ...[
              const SizedBox(height: 8),
              const Text(
                'Kelas berwarna abu-abu sudah dipetakan untuk mata pelajaran ini pada guru lain.',
                style: TextStyle(fontSize: 12, color: AppColors.gray500),
              ),
            ],
            const SizedBox(height: 16),
            const Text(
              'Kuota Jam per Minggu',
              style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500),
            ),
            const SizedBox(height: 8),
            TextField(
...
class _ScheduleFormModalState extends State<ScheduleFormModal> {
  ...
  String? _validateSchedule() {
    ...
    if (start < ss) return 'Jam pelajaran tidak boleh dimulai sebelum 07:00.';
    if (end > se)
  ...
  Future<void> _load() async {
    try {
      ...
      if (mounted)
        setState(() {
          _mapelList = ((results[0]['data'] as List?) ?? [])
              .cast<Map<String, dynamic>>();
          _guruMapelList = ((results[1]['data'] as List?) ?? [])
              .cast<Map<String, dynamic>>();
          _loading = false;
        });
    } catch (_) {
  ...
  String get _roomDisplay {
    ...
    return cap.toString().isNotEmpty
        ? '$code (Kapasitas: $cap)'
        : code.toString();
  }
  ...
  List<Map<String, dynamic>> get _filteredGuru {
    ...
    final matches = _guruMapelList.where((gm) {
  ...
  Widget build(BuildContext context) {
    ...
    final safeMapelId = _mapelList.any((e) => e['id'] == _selectedMapelId)
        ...
        : null;
    final safeGuruId =
        filteredGuru.any((g) => g['teacherId'] == _selectedGuruId)
        ? _selectedGuruId
        : null;
