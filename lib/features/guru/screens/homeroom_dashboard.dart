...
class _HomeroomDashboardState extends State<HomeroomDashboard> {
  ...
  Widget build(BuildContext context) {
    ...
    final attendanceStats = _data['attendanceStats'] as Map<String, dynamic>? ?? {};
    final totalHadir = attendanceStats['hadir'] ?? 0;
    final totalAll = totalHadir + (attendanceStats['sakit'] ?? 0) + (attendanceStats['izin'] ?? 0) + (attendanceStats['alpa'] ?? 0);
    final attendanceRate = totalAll > 0 ? ((totalHadir / totalAll) * 100).round() : 100;

    // Overall grade average
    final allGrades = students.map<double>((s) => (s['averageGrade'] ?? 0).toDouble()).where((g) => g > 0).toList();
    final overallGrade = allGrades.isNotEmpty ? (allGrades.reduce((a, b) => a + b) / allGrades.length).toStringAsFixed(1) : '0';

    final kpiData = [
      {'title': 'Rata-rata Kehadiran', 'value': '$attendanceRate%', 'color1': const Color(0xFF10B981), 'color2': const Color(0xFF059669), 'textColor': const Color(0xFF059669), 'icon': Icons.trending_up},
      {'title': 'Rata-rata Nilai Kelas', 'value': overallGrade, 'color1': const Color(0xFF3B82F6), 'color2': const Color(0xFF2563EB), 'textColor': const Color(0xFF2563EB), 'icon': Icons.bar_chart},
      {'title': 'Siswa Perlu Perhatian', 'value': '${flaggedStudents.length}', 'subtitle': 'Siswa', 'color1': const Color(0xFFEF4444), 'color2': const Color(0xFFB91C1C), 'textColor': const Color(0xFFB91C1C), 'icon': Icons.warning_amber},
    ];
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          // Header
          Text('Pantauan Akademik $kelas', style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w700, color: AppColors.foreground)),
          const SizedBox(height: 8),
          Text('$totalSiswa siswa terdaftar • Monitoring performa akademik dan kehadiran', style: const TextStyle(color: AppColors.gray600)),
                    ...
                    children: [
                      ...
                      if (subjectData.isEmpty)
                        Container(
                          ...
                          child: const Text('Belum ada data nilai', style: TextStyle(color: AppColors.gray500)),
                        )
                      else
                        Container(
                          height: 320,
                                ...
                                bottomTitles: AxisTitles(
                                  sideTitles: SideTitles(
                                    ...
                                    getTitlesWidget: (value, meta) {
                                      if (value.toInt() >= subjectData.length) return const SizedBox();
                          ...
                          children: subjectData.asMap().entries.map((e) {
                            ...
                            return AnimatedContainer(
                              ...
                              child: Row(
                                ...
                                children: [
                                  Expanded(child: Text(s.$1, style: TextStyle(fontSize: 13, fontWeight: isTouched ? FontWeight.bold : FontWeight.w500, color: isTouched ? AppColors.accent : AppColors.gray700), overflow: TextOverflow.ellipsis)),
                    ...
                    children: [
                      ...
                      if (flaggedStudents.isEmpty)
                        const Padding(
                          ...
                          child: Center(child: Text('Tidak ada siswa bermasalah ✓', style: TextStyle(color: AppColors.green700, fontWeight: FontWeight.w600))),
                        )
                      else
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Text(student['name'] ?? '-', style: const TextStyle(fontWeight: FontWeight.w700, color: AppColors.foreground)),
